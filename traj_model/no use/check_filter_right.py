...
for traj_file in traj_file_list:
            ...
            for key_theta, value_theta in value_y.items():
                for key_sv, value_sv in value_theta.items():
                    for key_ev, value_ev in value_sv.items():
                        ...
                        traj_library[library_key] = {}
                        traj_library[library_key]['label'] = label_dict 
                        traj_library[library_key]['trajectory'] = value_ev
                        
    TRAJ_LIBRARY_PATH_STANDARD = '/home/SENSETIME/zhoutong/hoffnung/vae_trajectory/data/vae_traj_matfiles/'
    pkl_name = 'final_vae_version.pickle'
    ...
    for key_x, value_x in traj_mat.items():
        print('value x: {}'.format(key_x))
        for key_y, value_y in value_x.items():
            print('     value y: {}'.format(key_y))
            for key_theta, value_theta in value_y.items():
                print('          value theta: {}'.format(key_theta))
                for key_sv, value_sv in value_theta.items():
                    print('               value start vel: {}'.format(key_sv))
                    for key_ev, value_ev in value_theta.items():
                        print('                         value end vel: {}'.format(key_ev))
                        break 
                    break 
                break 
            break 
