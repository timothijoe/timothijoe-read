...
def train_vae(model, train_loader, val_loader, params):
    ...
    for epoch in range(params.n_epochs):
        ...
        for train_init, train_traj, traj_type, traj_mask in tqdm(train_loader, desc = decs):
            ...
            recons = generate_compact_traj(ret[0], train_init, params.seq_len)
            expers = generate_compact_traj(ret[1], train_init, params.seq_len)
            name = ['train_expert', 'reconstruct', exp_name, str(epoch)]
            fake_epoch = iter_num // 50
            
            ret = model.loss_function(*ret, traj_mask)
            loss = ret['loss']
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if epoch > 0 and epoch % params.val_freq == 0:
            model.eval()
            total_loss = {}
            for val_init, val_traj, traj_type, traj_mask in tqdm(val_loader, desc = 'Val  '):
                with torch.no_grad():
                    ...
                    loss = model.loss_function(*ret, traj_mask)
